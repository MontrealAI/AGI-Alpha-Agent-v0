...
**Troubleshooting:** If tests fail due to missing optional packages, rerun `python check_env.py --auto-install` with `ALPHA_FACTORY_FULL=1`. When running without internet or behind strict firewalls, set `WHEELHOUSE=$(pwd)/wheels` and supply `--wheelhouse "$WHEELHOUSE"` so installs pull from the local wheel cache instead of PyPI.
