...
if [[ -n "${PYTEST_CURRENT_TEST:-}" || "${ALPHA_FACTORY_SKIP_CHECK_ENV:-0}" == "1" ]]; then
  echo "⚠️  Skipping environment check in test/override mode." >&2
elif [[ -f ../check_env.py ]]; then
...
if [[ -n "${PYTEST_CURRENT_TEST:-}" || "${ALPHA_FACTORY_SKIP_CHECK_ENV:-0}" == "1" ]]; then
  echo "⚠️  Skipping MuZero dependency check in test/override mode." >&2
...
printf '\n🎉  Open http://localhost:%s for the live MuZero dashboard.\n' "$HOST_PORT"
printf '🛑  Stop → docker compose -p alpha_muzero down\n'
