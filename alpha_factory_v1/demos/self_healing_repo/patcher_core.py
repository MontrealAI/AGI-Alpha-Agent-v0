...
def generate_patch(test_log: str, llm: OpenAIAgent, repo_path: str) -> str:
    """Return a patch generated by the LLM (sync wrapper)."""
...
def _apply_with_git(patch_file: str, repo_path: str) -> tuple[int, str]:
    return _run(["git", "-C", repo_path, "apply", "--unidiff-zero", patch_file], cwd=repo_path)


def _apply_with_patch(patch_file: str, repo_path: str) -> tuple[int, str]:
    return _run(["patch", "-p1", "-i", patch_file], cwd=repo_path)
...
def apply_patch(patch: str, repo_path: str) -> None:
    ...
    if shutil.which("patch") is None and shutil.which("git") is None:
        raise RuntimeError(
            "Neither `patch` nor `git` is available. Install one of them to apply patches."
        )
    backups = {}
    ...
    try:
        ...
        # apply
        apply_cmd: Callable[[str, str], tuple[int, str]]
        if shutil.which("git") and (repo / ".git").exists():
            apply_cmd = _apply_with_git
        else:
            apply_cmd = _apply_with_patch
        code, out = apply_cmd(patch_file, repo_path)
        if code != 0:
