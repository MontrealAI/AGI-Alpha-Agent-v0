...
def test_triage_permission_is_report_only() -> None:
    ...
    assert result.decision == TriageDecision.PERMISSION_OR_FORK_CONTEXT
...
def test_triage_does_not_flag_assignment_as_unsafe() -> None:
    ...
    result = triage_bundle(bundle)
    assert result.decision == TriageDecision.SAFE_AUTOPATCH
...
def test_engine_restores_repo_between_candidates(tmp_path: pathlib.Path) -> None:
    ...
    with (
        ...
        report = RepoHealerEngine(repo, EngineOptions(dry_run=False, max_attempts=2)).run(
            bundle, [bad_patch, good_patch]
        )
