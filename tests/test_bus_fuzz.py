...
json_values = st.recursive(
    ...
    lambda children: st.one_of(
        ...
        st.dictionaries(st.text(alphabet=ALPHABET, min_size=1, max_size=5), children, max_size=3),
    ),
    max_leaves=5,
)
...
def envelopes(draw: st.DrawFn) -> messaging.Envelope | types.SimpleNamespace:
    ...
    if as_proto:
        sender = draw(st.text(alphabet=ALPHABET, min_size=0, max_size=5))
        recipient = draw(st.text(alphabet=ALPHABET, min_size=0, max_size=5))
        ts = draw(st.floats(allow_nan=False, allow_infinity=False))
        payload: dict[str, Any] = draw(
            st.dictionaries(st.text(alphabet=ALPHABET, min_size=1, max_size=5), json_values, max_size=3)
        )
        if big_payload:
            payload["data"] = draw(st.text(alphabet=ALPHABET, max_size=500))
        env = messaging.Envelope(sender=sender, recipient=recipient, ts=ts)
        env.payload.update(payload)
        return env
    sender = draw(st.one_of(st.text(alphabet=ALPHABET, min_size=0, max_size=5), st.integers(), st.none()))
    recipient = draw(st.one_of(st.text(alphabet=ALPHABET, min_size=0, max_size=5), st.integers(), st.none()))
    ts = draw(
        st.one_of(
            st.floats(allow_nan=False, allow_infinity=False),
            st.text(alphabet=ALPHABET, min_size=0, max_size=5),
            st.none(),
        )
    )
    payload = draw(
        st.dictionaries(
            ...
            st.one_of(json_values, st.text(alphabet=ALPHABET, max_size=500)),
            max_size=3,
        )
    )
