...
def envelopes(draw: st.DrawFn) -> messaging.Envelope | types.SimpleNamespace:
    ...
    payload = draw(
        st.dictionaries(
            st.text(alphabet=SAFE_TEXT_ALPHABET, min_size=1, max_size=5),
            st.one_of(json_values, st.text(alphabet=SAFE_TEXT_ALPHABET, max_size=200)),
            max_size=3,
        )
    )
...
@settings(max_examples=_BUS_FUZZ_MAX_EXAMPLES, suppress_health_check=[HealthCheck.too_slow], deadline=None)
@given(env=envelopes())
