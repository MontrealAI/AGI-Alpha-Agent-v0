...
SAFE_TEXT_CHARS = st.sampled_from(list(SAFE_TEXT_ALPHABET))


def safe_text(min_size: int = 0, max_size: int = 20) -> st.SearchStrategy[str]:
    """Return a Hypothesis strategy for bounded ASCII-friendly text."""
    return st.lists(SAFE_TEXT_CHARS, min_size=min_size, max_size=max_size).map("".join)
...
def envelopes(draw: st.DrawFn) -> messaging.Envelope | types.SimpleNamespace:
    ...
    if as_proto:
        sender = draw(safe_text(max_size=5))
        recipient = draw(safe_text(max_size=5))
