...
contract ValidationModule is Ownable, IValidationModule {
    modifier onlyJobRegistry() {
        require(msg.sender == address(jobRegistry), "not job registry");
        _;
    }
    ...
    function validate(uint256 jobId, bytes calldata) external override onlyJobRegistry returns (bool) {
    ...
    function tally(uint256 jobId, uint256 reward, uint256 duration) external onlyJobRegistry {
        Round storage r = rounds[jobId];
