...
@given(
    ...
    payload=payloads(include_code=True, code_strategy=st.text(min_size=0, max_size=100).map(lambda s: f"import os{s}")),
)
...
@given(
    ...
    payload=payloads(
        include_code=True,
        code_strategy=st.text(min_size=0, max_size=100).map(lambda s: s.replace("import os", "import_os")),
    ),
)
