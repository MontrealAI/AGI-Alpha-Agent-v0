...
@settings(max_examples=30)
@given(code=st.text(min_size=0, max_size=100).map(lambda s: s.replace("import os", "importos")))
...
def payloads(draw: st.DrawFn, include_code: bool) -> dict[str, object]:
    ...
    if include_code:
        extra["code"] = draw(st.text(min_size=0, max_size=100))
        return extra
    return extra
...
def test_fuzz_envelope_allows_safe(sender: str, recipient: str, ts: float, payload: dict[str, object]) -> None:
    payload["code"] = str(payload["code"]).replace("import os", "importos")
    bus = DummyBus(config.Settings(bus_port=0))
    led = DummyLedger()
    agent = safety_agent.SafetyGuardianAgent(bus, led)
    env = messaging.Envelope(sender=sender, recipient=recipient, payload=payload, ts=ts)
