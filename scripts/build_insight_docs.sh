...
BINARY_EXT_REGEX='\\.(wasm|zip|gz|png|jpg|jpeg|webp|gif|pdf|ico|mp4|mp3|woff2?|ttf|otf|bin|exe|dll|so|dylib)$'

if [[ -n "${INSIGHT_ASSETS_ROOT:-}" && -z "${FETCH_ASSETS_ROOT:-}" ]]; then
    export FETCH_ASSETS_ROOT="$INSIGHT_ASSETS_ROOT"
fi
...
if [[ "${SKIP_BINARY_COPY:-}" == "1" ]]; then
    ...
    unzip -q -o "$BROWSER_DIR/insight_browser.zip" -d "$TEMP_DOCS_DIR"
    while IFS= read -r -d '' file; do
        rel="${file#"$TEMP_DOCS_DIR"/}"
        if [[ "$rel" =~ $BINARY_EXT_REGEX ]]; then
            continue
        fi
        target="$DOCS_DIR/$rel"
        mkdir -p "$(dirname "$target")"
        cp -a "$file" "$target"
    done < <(find "$TEMP_DOCS_DIR" -type f -print0)
    rm -rf "$TEMP_DOCS_DIR"
else
    rm -rf "$DOCS_DIR"
    mkdir -p "$DOCS_DIR"
