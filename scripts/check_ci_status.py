...
def _latest_run(repo: str, workflow: str, token: str | None, *, ref: str | None = None) -> Mapping[str, object]:
    query = "per_page=1"
    if ref:
        query = f"{query}&branch={ref}"
    ...
    *,
    ...
    while True:
        ...
        for workflow in workflows:
            try:
                run = _latest_run(repo, workflow, token, ref=ref)
