...
Current release: **1.5.2**. See [release readiness and deployment scope](RELEASE_READINESS.md) before choosing a launch path.
...
version. Do not have two versions operating on the same home. No journal migration is needed when upgrading from 1.2.0 through 1.5.2.
...
```sh
docker build --target agent-runtime -t agialpha-agent:1.5.2 -f alpha_factory_v1/Dockerfile .
docker run --name agialpha-agent -d -p 127.0.0.1:8000:8000 -v agialpha-data:/data agialpha-agent:1.5.2
...
The release includes `alpha-agent-v1.5.2-browser.zip`, the complete tested browser distribution.
