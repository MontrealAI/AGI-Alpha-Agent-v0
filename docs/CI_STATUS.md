...
| Integration CI | Latest push to main: Python tests, browser checks, docs and Docker validation | [Integration runs](https://github.com/MontrealAI/AGI-Alpha-Agent-v0/actions/workflows/ci.yml?query=branch%3Amain+event%3Apush) |
...
PR CI, Integration and acceptance use GitHub's branch and push-event selectors.
Health and Smoke use Shields' live check-runs endpoint with the exact `CI watchdog` and `Smoke matrix`
names and main branch. The matrix result runs even after a failure and rejects failure,
cancellation or skipped matrix jobs. Neither badge sets a status color or manufactures a passing
