...
1. **Branch protection**
   ...
   - **Permissions:** ensure the workflows checking branch protection (`ci.yml` and `ci-health.yml`) grant the Actions token read access to repository contents (plus `actions: write` for dispatch/cancel) so the verification step can read protection rules. GitHub now rejects unsupported permission keys such as `administration`, so rely on the default repository-level access granted to the workflow token instead of requesting invalid scopes.
