...
|3|`alpha_agi_business_2_v1`|🏗|Autonomous supply‑chain company builder.|Bootstraps logistics & manufacturing alpha.|`bash alpha_factory_v1/demos/alpha_agi_business_2_v1/run_business_2_demo.sh`|
...
|8|`era_of_experience`|🏛|Self‑improving RL playground & dataset builder.|Curates lifelong experience for generalist agents.|`docker compose -f demos/docker-compose.experience.yml up`|
...
|11|`muzero_planning`|♟|MuZero planning in 60 s, no rules required.|Model‑based RL with Monte‑Carlo search.|`bash alpha_factory_v1/demos/muzero_planning/run_muzero_demo.sh`|
...
|13|`meta_agentic_agi`|👁|Multi‑objective meta‑search evolves new agents.|Self‑improves via NSGA‑II & Reflexion.|`python alpha_factory_v1/demos/meta_agentic_agi/meta_agentic_agi_demo.py --provider mistral:7b-instruct.gguf`|
