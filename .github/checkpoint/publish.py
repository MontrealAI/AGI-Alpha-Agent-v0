...
def main() -> None:
    ...
    if os.getenv("GITHUB_REPOSITORY") != repo or os.getenv("GITHUB_REF") != "refs/heads/release/checkpoint-2026-09-25-v1.3.0-wip":
        raise RuntimeError("publication is restricted to the authorized checkpoint branch")
    sha = "198621c8bf59063e3afc2310bb24cd6c4aac8971"
    ...
    if ref.returncode == 0:
        ...
        annotated = json.loads(
            gh(
                ...
                f"tag={tag}",
                "-f",
                "message=$AGIALPHA Agent development checkpoint: exact current source and Git history; v1.3.0 work in progress preserved for continuation",
                "-f",
                f"object={sha}",
                "-f",
                "type=commit",
    ...
    if not prior:
        gh(
            ...
            "--prerelease",
            "--title",
            "$AGIALPHA Agent v1.3.0 WIP checkpoint — 2026-09-25",
            "--notes-file",
