...
def main() -> None:
    ...
    if os.getenv("GITHUB_REPOSITORY") != repo or os.getenv("GITHUB_REF") != "refs/heads/release/checkpoint-2026-09-24-v1.2.1-wip":
        raise RuntimeError("publication is restricted to the authorized checkpoint branch")
    sha = "c697b3333cb4051f424719e7094f8f859665f9b9"
    ...
    if ref.returncode == 0:
        ...
        annotated = json.loads(
            gh(
                ...
                f"tag={tag}",
                "-f",
                "message=$AGIALPHA Agent development checkpoint: exact current source and Git history; completion validation remains in progress",
                "-f",
                f"object={sha}",
                "-f",
                "type=commit",
    ...
    if not prior:
        gh(
            ...
            "--prerelease",
            "--title",
            "$AGIALPHA Agent checkpoint 2026-09-24",
            "--notes-file",
