...
export function clusterKeywords(runs: Array<{ keywords: string[] }>, thresh = 0.6): string[][] {
...
export async function validateLabel(name: string): Promise<boolean> {
  ...
  try {
    const resp = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-3.5-turbo',
        messages: [{ role: 'user', content: `Does '${name}' denote a distinct economic activity?` }],
      }),
    });
    const data = await resp.json();
    const content = data?.choices?.[0]?.message?.content ?? data?.choices?.[0]?.text ?? '';
    return /^yes/i.test(String(content).trim());
  } catch {
    return false;
  }
}
